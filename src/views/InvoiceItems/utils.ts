...
function filterInvoiceData(
  ...
  params: {vendor: string | null, startDate: Date | null, endDate: Date | null},
): InvoiceData[] {
  let filteredResults = invoiceData;
  if (params.vendor) {
    filteredResults = filteredResults.filter((invoice) => (
      invoice.Vendor === params.vendor));
  }

  // For data filters. The firestore data currently holds the dates as strings.
  // This has to be converted and compared until
